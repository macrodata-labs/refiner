...
def emit_unique_entries(
    *,
    entries: Any,
    emit_entry: Callable[[dict[str, Any]], None],
    seen_keys: set[_LogKey],
    seen_order: deque[_LogKey],
    limit: int,
) -> tuple[int | None, int | None]:
    oldest_emitted_ms: int | None = None
    newest_emitted_ms: int | None = None
    if not isinstance(entries, list):
        return oldest_emitted_ms, newest_emitted_ms
    ...
    for entry in entries:
        ...
        if parsed_entry_ms is None:
            continue
        if oldest_emitted_ms is None:
            oldest_emitted_ms = parsed_entry_ms
        newest_emitted_ms = parsed_entry_ms
...
class FollowLogPoller:
    ...
    def poll(
        ...
        *,
        ...
        self.retryable_error_count = 0
        oldest_emitted_ms, _ = emit_unique_entries(
...
class ForwardLogPoller:
    ...
    def reset_window(self, *, start_ms: int) -> None:
    ...
    def poll(
        ...
        *,
        ...
        end_ms = max(now_ms(), self.start_ms + 1)
        payload, self.retryable_error_count = call_with_retry(
            lambda: fetch_page(self.start_ms, end_ms, self.cursor),
            retryable_error_count=self.retryable_error_count,
            max_retryable_errors=self.max_retryable_errors,
        )
        self.retryable_error_count = 0
        ...
        if self.cursor is not None:
            ...
            if self.full_batch_polls < self.max_drain_polls:
                return FollowPollResult(action="drain")
            skipped_start_ms = newest_emitted_ms or self.start_ms
            skipped_end_ms = end_ms
            self.cursor = None
            self.full_batch_polls = 0
