...
def write_bundled_runtime() -> Path:
    runtime_dir = Path("/tmp/refiner-hawor-official-runtime")
...
def ensure_hawor_runtime():
    ...
    if not Path(HAWOR_EXPORT).exists() or not _hawor_renderer_available():
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "Official HaWoR setup failed with exit code "
                f"{completed.returncode}\n{_tail_output(completed.stdout, completed.stderr)}"
            )
    if not _hawor_renderer_available():
        raise RuntimeError(
            "HaWoR renderer is unavailable after official setup; PyTorch3D is required."
        )
    _HAWOR_READY = True
...
def reconstruct_hawor(row):
    ...
    output_dir = Path("/tmp/hawor-official-artifacts") / "sample"
...
if __name__ == "__main__":
    pipeline.launch_cloud(
        name=os.environ.get("REFINER_JOB_NAME", "hawor-egocentric-official-inline"),
