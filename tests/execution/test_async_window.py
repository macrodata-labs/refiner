...
def test_async_window_submit_blocking_enforces_max_in_flight() -> None:
    ...
    assert window.submit_blocking(_delayed_value(1, 0.03)) == []

    start = time.perf_counter()
    completed = window.submit_blocking(_delayed_value(2, 0.0))
...
def test_async_window_poll_returns_ready_results_without_blocking() -> None:
    ...
    assert window.submit_blocking(_delayed_value(1, 0.03)) == []
    assert window.submit_blocking(_delayed_value(2, 0.0)) == []
